...
void fill_screen(uint8_t r, uint8_t g, uint8_t b) {
    ...
    uint32_t *vidmemcur = (uint32_t *)(current_buffer.graphics_vid_buffer);
    for (uint32_t i = 0; i < (current_buffer.buffer_height * current_buffer.buffer_width);
         i++) {
...
void render8x8bitmap(
    unsigned char bitmap[8], uint8_t xpos, uint8_t ypos, color_t bg, color_t fg) {
    char current_map;
    uint32_t foreground_color =
        (fg.red << (red_byte)) | (fg.green << (green_byte)) | (fg.blue << (blue_byte));
    uint32_t background_color =
        (bg.red << (red_byte)) | (bg.green << (green_byte)) | (bg.blue << (blue_byte));
...
void update_display() {
    uint32_t *vidmemcur = (uint32_t *)(((uint32_t *)vidmem) +
        (current_buffer.x +
            (current_buffer.y * width))); // Video memory offset to draw the buffer at
    uint32_t *buffer_mem =
        (uint32_t *)current_buffer.graphics_vid_buffer; // The buffer's memory to read from

    for (uint32_t i = 0; i < current_buffer.buffer_height;
         i++) { // Iterate over the whole buffer
        memcpy32(buffer_mem, vidmemcur,
            current_buffer.buffer_width); // Copy the current line to video memory
