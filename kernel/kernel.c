...
void kmain(multiboot_info_t *mbd, uint32_t end_of_code) {
    ...
    sprintf("\n\nVESA info:\n  Colors: %u\n  Red pos: %u\n  Green pos: %u\n  Blue pos: %u",
        (uint32_t)mbd->framebuffer_palette_num_colors,
        (uint32_t)mbd->framebuffer_red_field_position,
        (uint32_t)mbd->framebuffer_green_field_position,
        (uint32_t)mbd->framebuffer_blue_field_position);
    sprintf("\n  Framebuffer pitch: %u\n  Framebuffer size: %lu\n  Framebuffer width: %lu",
        mbd->framebuffer_pitch,
        (uint64_t)(mbd->framebuffer_height * mbd->framebuffer_pitch),
    ...
    if (mbd->flags & MULTIBOOT_INFO_MEM_MAP) {
        ...
        //          pixel < mbd->framebuffer_width * mbd->framebuffer_height; pixel++) {
        //         RgbColor rgb_dat = HsvToRgb(color_dat);
        //         // sprintf("\nHSV: %u %u %u\nRGB: %u %u %u", (uint32_t) color_dat.h,
        //         // (uint32_t) color_dat.s, (uint32_t) color_dat.v, (uint32_t) rgb_dat.r,
        //         // (uint32_t) rgb_dat.g, (uint32_t) rgb_dat.b); sprintf("\n\nwriting to pos:
        //         // %lx", mbd->framebuffer_addr + (pixel * 4));
        //         *((uint32_t *)(mbd->framebuffer_addr + (pixel * 4))) =
        //             (uint32_t)((uint32_t)rgb_dat.r << mbd->framebuffer_red_field_position) |
        //             ((uint32_t)rgb_dat.g << mbd->framebuffer_green_field_position) |
        //             ((uint32_t)rgb_dat.b << mbd->framebuffer_blue_field_position);
