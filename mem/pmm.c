...
uint8_t *phys_to_bitmap(uint64_t phys_addr) {
    ...
    while (map_iterator) {
        if (get_represented_addr(map_iterator) - NORMAL_VMA_OFFSET <= phys_addr &&
            phys_addr <= get_represented_addr(map_iterator) +
                    (get_bitmap_size(map_iterator) * 0x1000 * 8) - NORMAL_VMA_OFFSET) {
...
void set_bitmap(
    ...
    if (old_bitmap) { // If old_bitmap is not 0
        ...
        uint64_t old_bitmap_size = *(uint64_t *)old_bitmap;
        *(uint64_t *)((uint64_t)old_bitmap + old_bitmap_size) =
...
void configure_mem(multiboot_info_t *mbd) {
        ...
        if (mmap->type == MULTIBOOT_MEMORY_AVAILABLE) {
            if (remaining == mbd->mmap_length) {
                ...
                if ((uint64_t)__kernel_start - KERNEL_VMA_OFFSET >= (mmap->addr) &&
                    ((uint64_t)__kernel_end - KERNEL_VMA_OFFSET) <
                        (mmap->len + mmap->addr) -
                            ((mmap->len + mmap->addr) / 0x1000 / 8)) {
                    // The kernel is here and so we setup the bitmap
                    bitmap = (uint8_t *)(((((uint64_t)__kernel_end - KERNEL_VMA_OFFSET) +
                                              NORMAL_VMA_OFFSET) +
                                             0x1000) &
                        ~(0xfff));
                    set_bitmap(bitmap, 0, mmap->len,
                        (((uint64_t)__kernel_end + 0x1000 - KERNEL_VMA_OFFSET) & ~(0xfff)) -
                            (mmap->addr));
                    sprintf("\nFirst bitmap set!");
                    break;
                }
    ...
    // TODO: update loops and also map stuff in the second loop and make the first loop
    // smaller
    ...
    for (; remaining > 0; remaining -= sizeof(multiboot_memory_map_t)) {
        ...
        if (mmap->type == MULTIBOOT_MEMORY_AVAILABLE) {
            ...
            if ((uint64_t)__kernel_start - KERNEL_VMA_OFFSET >= (mmap->addr) &&
                ((uint64_t)__kernel_end - KERNEL_VMA_OFFSET) <
                    (mmap->len + mmap->addr) - ((mmap->len + mmap->addr) / 0x1000 / 8)) {
                vmm_remap((void *)mmap->addr, (void *)(mmap->addr + NORMAL_VMA_OFFSET),
                    (mmap->len + 0x1000 - 1) / 0x1000, 0);
                vmm_flush_tlb();
            ...
            for (uint64_t bits_iterated = 0; bits_iterated < 8; bits_iterated++) {
                ...
                if (!get_bit(cur_map, bits_iterated, bytes_iterated)) {
                    ...
                    if (!first_addr) {
                        ...
                        first_addr = (get_represented_addr(cur_map) +
                            (((bytes_iterated * 8) + (bits_iterated)) * 0x1000));
                    }
                } else {
                    found_free_bits = 0;
                    first_addr = 0;
...
void pmm_unallocate(void *address, uint64_t size) {
    ...
    uint64_t difference =
        (uint64_t)address - (get_represented_addr(bitmap_to_free) - NORMAL_VMA_OFFSET);
    ...
    if ((size_bytes + difference_bytes) > bitmap_size) {
        sprintf("\n[PMM] Warning: Trying to free too much space! (%lu bitmap bytes from "
                "size %lu and from bitmap at %lx)",
            size_bytes, bitmap_size, bitmap_to_free);
        return;
    }
