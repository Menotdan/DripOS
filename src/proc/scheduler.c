...
void scheduler_init_bsp() {
    ...
    starting_task.regs.fs = (uint64_t) kmalloc(sizeof(thread_info_block_t));
    ...
    dynarray_add(task_t, tasks, &starting_task);
    kprintf("\n[SCHEDULER] Added item");
}
...
void schedule(int_reg_t *r) {
    ...
    for (uint64_t i = 0; i < item_count; i++) {
        ...
        if (task->times_selected < lowest_selected_count && i == 0) {
