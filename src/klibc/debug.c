...
void set_debug_state() {
    ...
    if (get_cur_process()) {
        write_debug_register('2', get_cur_process()->local_watchpoint1);
        write_debug_register('3', get_cur_process()->local_watchpoint2);
    } else {
...
void set_local_watchpoint(void *address, uint8_t watchpoint_index) {
    if (watchpoint_index == 0) {
        get_cur_process()->local_watchpoint1 = (uint64_t) address;
        get_cur_process()->local_watchpoint1_active = 1;
    } else if (watchpoint_index == 1) {
        get_cur_process()->local_watchpoint2 = (uint64_t) address;
        get_cur_process()->local_watchpoint2_active = 1;
    }
